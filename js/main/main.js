...
const toSelect2 =document.getElementById("currency_to_dropdown");
const fromSelect = document.getElementById("currency_from_dropdown");
const fromSelect2 =document.getElementById("currency_from_dropdown");
...
const todayRate = document.getElementById('today_rate');
        ...
        { 
            if (response)
            {
                response.json().then((jsonData) => {
                    let todayRate = Math.floor(jsonData["rates"].IDR / jsonData["rates"].JPY * 100)/100;

                    updateRate(todayRate);
                });
            }
            ...
            option3.value = 'IDR';
            option4.value ='JPY';
            option3.text = '(IDR) Indonesian Rupiah';
            option4.text = '(JPY) Japanese Yen';
            

            toSelect2.add(option3,null);
            fromSelect2.add(option4, null);
